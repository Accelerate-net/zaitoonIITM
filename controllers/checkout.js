...
.controller('CheckoutController', ['$scope','$http', '$cookies', function($scope, $http, $cookies) {
   ...
   if(localStorage.getItem("pickupOutlet")){
    ...
    $scope.onOutletChange = function(outlet){
      ...
      $http.get('https://www.accelerateengine.app/food-engine/apis/fetchoutletsweb.php?outletcode='+outlet)
    ...
    $scope.initAddress = function(){    		
	       ...
	       $http({
	        method  : 'POST',
	        url     : 'https://www.accelerateengine.app/food-engine/apis/addresssuggestions.php',
	        data    : data, //forms user object
	        headers : {'Content-Type': 'application/x-www-form-urlencoded'}
	       })
	        .then(function(response) {
    ...
    $scope.saveNewAddressToList = function(){
        ...
        if($scope.newAddressContent.name == ""){
          ...
          $scope.newError = "Contact Number can not be empty";
	       ...
	       $http({
	        method  : 'POST',
	        url     : 'https://www.accelerateengine.app/food-engine/apis/newaddress.php',
	        data    : data, //forms user object
	        headers : {'Content-Type': 'application/x-www-form-urlencoded'}
	       })
	        .then(function(response) {
    ...
    $scope.deleteThisAddress = function(uid){
	       ...
	       $http({
	        method  : 'POST',
	        url     : 'https://www.accelerateengine.app/food-engine/apis/deleteaddress.php',
	        data    : data, //forms user object
	        headers : {'Content-Type': 'application/x-www-form-urlencoded'}
	       })
	        .then(function(response) {
      ...
      $scope.checkout = function(checkoutAddress, comments, coupon){
	    ...
	    $('#loading').show();
            ...
            $http({
              method  : 'POST',
              url     : 'https://www.accelerateengine.app/food-engine/apis/createorderweb.php',
              data    : data,
              headers : {'Content-Type': 'application/x-www-form-urlencoded'}
             })
              .then(function(response) {
				    	...
				    	if(timeLeft == 0)
				    	{
				    		...
				    		setTimeout(function(){
				    			...
				    			if(attemptsCount == 5){
				    				...
				    				document.getElementById("paymentNote").innerHTML = "Create a REFUND REQUEST from Help menu Or contact hello@thezaitoon.com with Reference ID ("+data.transactionID+") for refunding the debited amount.";
	                            ...
	                            $http({
	                              method  : 'POST',
	                              url     : 'https://www.accelerateengine.app/food-engine/apis/processpaymentweb.php',
	                              data    : data,
	                              headers : {'Content-Type': 'application/x-www-form-urlencoded'},
	                              timeout : 18000
	                             })
